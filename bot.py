...
async def outbox_loop(bot: Bot):
    """Опрашивает очередь bot_outbox и шлёт уведомления (попытки чужого входа и т.п.).
    Если chat_id пуст — отправляем всем админам из TG_ADMIN_IDS."""
    ...
    while True:
        try:
            ...
            for row in rows:
                ...
                # Помечаем отправленным, даже если нет адресатов — чтобы не зацикливаться.
                if delivered or not targets:
